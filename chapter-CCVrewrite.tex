...
\begin{multline} \label{CCV-energy inequality generic}
\ddt \int (u - \psi)_+^2 + \norm{(u-\psi)_+}^2_{H^{s/2}(\R^d)} - B[(u-\psi)_+,(u-\psi)_-] 
\\ \leq C \bracket{\int (u-\psi)_+^2 + \iint \indic{u-\psi_1 > 0}(x)[\psi(y)-\psi(x)]^2 \,dxdy }
\end{multline}
...
A_1(t) &= \{x\in B_3: \phi_1 \leq u(x,t)\}.
\end{align*}
...
\end{equation}
\[ \abs{\{u \leq 0\} \cap (-3,-2)\times B_1} \geq |B_1|, \]
and
\[ \abs{\{u \geq 1 + (1/k)F\} \cap (-2,0)\times B_3} \leq \delta, \]
but
